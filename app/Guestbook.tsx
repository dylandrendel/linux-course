...
export default function GuestbookSigner() {
  ...
  return (
    <div>
      {user && (
        <button
          className="w-full py-2 px-4 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-lg shadow-md focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-opacity-75"
        ...
        {signatures.map((signature) => (
          <li key={signature.id} className="text-sm text-gray-600">
            <span className="font-semibold">
              {signature.expand?.user?.username || "Anonymous"}
            </span>
            : "{signature.message}"
            <span className="text-xs text-gray-400">
              {" "}
              - {new Date(signature.created).toLocaleString()}
            </span>
          </li>
        ))}
      </ul>
